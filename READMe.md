...
- 대표 수행 업무:
  ...
  - xcode 프로젝트 환경 파일로인한 git 충돌 있슈 -> xcodegen 도입으로 해결 (2021.11.15 ~ 2021.11.17)
    - 일주일 주기로 배포 시 xcode 환경 파일에 의해 git brand 합치는 과정에서 merge 충돌로 인한 40분 정도의 비효율적인 시간이 걸리는 것을 파악한 후 xcode가 자동으로 만들어주는 환경파일을 xcode에 맡기지 않고 xcodegen을 통해 생성 시키므로써 프로젝트 환경 파일에 따른 git merge 충돌 시에 걸렸던 시간을 제거하여 생산성을 높임
