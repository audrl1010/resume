...
### 레벨 13
- 기간: 2021.08 ~ 현재
- 내용: 남자 쇼핑 플랫폼 룩핀 e-commerce 서비스 iOS 개발
...
- 대표 수행 업무:
  ...
  - 홈 개편 (2021.11.18 ~ 12.18)
    - 카테고리 확장 및 브랜드 활성화를 위한 토대 마련
    - 홈 텝안에 브랜드 텝, 랭킹 텝, 세일 텝을 작업

  - xcode 프로젝트 환경 파일로인한 git 충돌 있슈 -> xcodegen 도입으로 해결 (2021.11.15 ~ 2021.11.17)
    - 매주 배포 시 xcode 환경 파일에 의해 git brand 합치는 과정에서 merge 충돌로 인한 40분 정도의 비효율적인 시간(한달이면 40분 * 4 = 160분)이 걸리는 것을 파악한 후 xcode가 자동으로 만들어주는 환경파일을 xcode에 맡기지 않고 xcodegen을 통해 생성 시키므로써 프로젝트 환경 파일에 따른 git merge 충돌 시에 걸렸던 시간을 제거하여 생산성을 높임

  - 유저 데이터 추적을 위한 log 심는 작업 (2021.11.06 ~ 2021.11.14)
    - 유저 데이터를 추적하여 정확한 매출 및 의미 있고 개선할 수 있는 데이터 분석을 가능 하도록 함
  
  - 카테고리 상세 페이지에 연관 검색 키워드 추가 작업 (2021.11.03 ~ 2021.11.05)
    - 하위 카테고리에 연관 키워드 추천 리스트를 추가하여 좀 더 상세한 필터링을 할 수 있도록 하여 유저의 구매 증진시킴

  - 코디 태그 개선 (2021.10.18 ~ 2021.11.06)
    - 코디 기능 강화, 트래픽 비중 증가
  
  - 상품 상세 페이지 속도 개선 (2021.09.24 ~ 2021.09.26)
