...
- 개선사항:
  ...
  - Xcodegen을 이용 해 깃에서 자주 발생하는 프로젝트 설정 파일 충돌 문제를 해결 했습니다.
  - 모든 유틸리티는 유닛 테스트를 반드시 작성하였고, 테스트가 반드시 필요한 결제, 주문 기능에서는 단위 테스트, 통합 테스트를 작성하여 서비스의 안전성을 높였습니다.
  - fastlane을 사용하여 파이어베이스 앱 배포를 자동화 시켰습니다.
...
- 개선사항:
  - Code로 UI 개발 시 AutoLayout과 StackView 보다 더 빠르고 수정이 쉬운 개발을 위해 Texture를 도입하였고
    UI개발에 대한 작업 시간을 많이 단축시켰습니다.
  - ViewController에 Navigation(이동) 코드를 제거하여, ViewController의 재사용을 높이기 위해
    RIBs를 도입하여 ViewController의 재사용성을 높였습니다.
